...
const AdminDashboard = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      if (activeTab === 'charities') {
        ...
        // Mocked as requested
        setUsers([
          { _id: '1', name: 'John Doe', email: 'john@example.com', role: 'user', subscriptionStatus: 'active' },
          { _id: '2', name: 'Jane Smith', email: 'jane@example.com', role: 'admin', subscriptionStatus: 'active' },
          { _id: '3', name: 'Bob Wilson', email: 'bob@example.com', role: 'user', subscriptionStatus: 'inactive' },
        ]);
      }
    } catch (err) {
      console.error(err);
  ...
  const handleAddCharity = async (e) => {
    ...
    try {
      ...
      setStatus({ type: 'error', text: err.response?.data?.message || 'Failed to add charity' });
    } finally {
  ...
  const publishDraw = async () => {
    if (!window.confirm('Are you sure you want to publish this draw? This will notify all winners.')) return;
    setIsSubmitting(true);
    try {
      ...
      <main className="ml-64 flex-1 p-8">
        <div className="mx-auto max-w-5xl">
          ...
          <header className="mb-12 flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-extrabold tracking-tight text-white capitalize">{activeTab}</h1>
              <p className="mt-2 text-neutral-500">Manage your system-wide {activeTab} operations.</p>
            </div>
            {status.text && (
              <div className={`flex items-center gap-2 rounded-full px-4 py-2 text-sm border ${
                status.type === 'success' 
              ...
              <div className="lg:col-span-1">
                <div className="rounded-2xl border border-neutral-800 bg-neutral-900 p-6">
                  ...
                  <form onSubmit={handleAddCharity} className="space-y-4">
                    ...
                    >
                      {isSubmitting ? <Loader2 className="h-5 w-5 animate-spin mx-auto" /> : 'Register Charity'}
                    </button>
                  </form>
                </div>
              </div>
              ...
              <div className="lg:col-span-2">
                <div className="grid gap-4 md:grid-cols-2">
                  {charities.map((charity) => (
                    <div key={charity._id} className="group relative overflow-hidden rounded-2xl border border-neutral-800 bg-neutral-900 p-6 transition-all hover:border-neutral-700">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-neutral-500 line-clamp-2">{charity.description}</p>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div className="grid gap-8 lg:grid-cols-3">
                  <div className="lg:col-span-2 space-y-6">
                    <div className="rounded-2xl border border-neutral-800 bg-neutral-900 p-8">
                      <h3 className="text-xl font-bold mb-6">Winning Numbers</h3>
                      <div className="flex gap-3">
                        {simulation.numbers.map((num, i) => (
                          <div key={i} className="flex h-16 w-16 items-center justify-center rounded-2xl bg-emerald-500 text-2xl font-black text-white shadow-xl shadow-emerald-500/20">
                            {num}
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="rounded-2xl border border-neutral-800 bg-neutral-900 p-8">
                      <h3 className="text-xl font-bold mb-6 text-white">Tier Breakdown</h3>
                      <table className="w-full text-left">
                        <thead>
                          <tr className="border-b border-neutral-800 text-xs uppercase tracking-widest text-neutral-500">
                            ...
                            <th className="pb-4 font-medium">Potential Winners</th>
                            <th className="pb-4 font-medium">Payout / Person</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-neutral-800">
                          {simulation.tiers.map((tier) => (
                            <tr key={tier.matchCount}>
                              <td className="py-4 font-semibold">{tier.matchCount} Matches</td>
                              <td className="py-4 text-emerald-500 font-bold">{tier.winnerCount}</td>
                              <td className="py-4 font-mono">£{tier.payout.toFixed(2)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                  ...
                  <div className="space-y-6">
                    <div className="rounded-2xl border border-indigo-500/20 bg-indigo-500/5 p-8">
                      <p className="text-xs uppercase tracking-widest text-indigo-400 font-bold mb-2">Total Prize Pool</p>
                      <h2 className="text-5xl font-black text-white">£{simulation.prizePool.toFixed(2)}</h2>
                      <p className="mt-4 text-sm text-indigo-300">Total participants: {simulation.totalPlayers}</p>
                    </div>
                    <button 
                      onClick={publishDraw}
                      disabled={isSubmitting}
          ...
          {activeTab === 'users' && (
            <div className="rounded-2xl border border-neutral-800 bg-neutral-900 overflow-hidden">
               <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-neutral-800">
                  {users.map((user) => (
                    <tr key={user._id} className="hover:bg-neutral-800/30 transition-all">
                      <td className="px-8 py-6">
                        <div className="font-bold">{user.name}</div>
                        <div className="text-sm text-neutral-500">{user.email}</div>
                      </td>
                      <td className="px-8 py-6">
                        <span className={`rounded-full px-3 py-1 text-xs font-bold uppercase tracking-tighter ${
                          user.subscriptionStatus === 'active' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'
                        }`}>
                          {user.subscriptionStatus}
                        </span>
                      </td>
                      <td className="px-8 py-6">
                        <span className="text-sm text-neutral-300 capitalize">{user.role}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
